...
var Wibble = class extends CustomType {
};
var Wobble = class extends CustomType {
};
...
function on_url_change(uri) {
  ...
  if ($.hasLength(1) && $.head === "wibble") {
    return new OnRouteChange(new Wibble());
  } else if ($.hasLength(1) && $.head === "wobble") {
    return new OnRouteChange(new Wobble());
  } else {
    return new OnRouteChange(new Wibble());
  }
}
function init3(_) {
  return [new Wibble(), init2(on_url_change)];
}
    ...
    toList([
      nav(
        ...
        toList([
          a(
            toList([href("/wibble")]),
            toList([text("Go to wibble")])
          ),
          a(
            toList([href("/wobble")]),
            toList([text("Go to wobble")])
          )
        ])
      ),
      (() => {
        if (route instanceof Wibble) {
          return h1(
            toList([]),
            toList([text("You're on wibble")])
          );
        } else {
          return h1(
            toList([]),
            toList([text("You're on wobble")])
          );
        }
      })()
    ])
...
function main() {
  debug("Starting client controller");
